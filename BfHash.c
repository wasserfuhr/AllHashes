...
int main(void){
 ...
 do{
   ...
   do{
    ...
    do{
     fprintf(stdout,"%-40s ______%02x%02x%02x %ju\n","",j,k,l,(uintmax_t)time(NULL));
     fflush(stdout);
     unsigned char i=0;
     do{
      data[2]=i;
   ...
   do{
    ...
    do{
     ...
     if (hash[j]<mhash[j]) {
      ...
      fprintf(stdout,"%s %02x%02x%02x%02x%02x%02x %ju\n", buf, k,l,data[2],data[3],data[4],data[5], (uintmax_t)time(NULL));
      fflush(stdout);
      break;
     }
     j++;
